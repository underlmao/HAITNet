...
parser.add_argument('--method', type=str, default='PVT_CAMO1000')
...
for dataset in tqdm(test_datasets):
    ...
    loader = eval_Dataset(osp.join(pred_dir, method, dataset), osp.join(gt_dir, dataset, 'GT/'))
...
log_path = os.path.join(eval_root, method)
check_mkdir(log_path)
result_file = os.path.join(log_path, str(method) + '.txt')
open(result_file, 'w').write(str(result_table) + '\n\n')
print("Finish evaluation for method {}, all the files saved in {}".format(method, eval_root))